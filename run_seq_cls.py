...
def train(args,
    ...
    for epoch in tqdm(mb, desc='epcoh'):
        epoch_iterator = progress_bar(train_dataloader, parent=mb)
        for step, batch in enumerate(tqdm(epoch_iterator, total=len(train_dataloader), desc='iteration')):
            model.train()
...
def evaluate(args, model, eval_dataset, mode, global_step=None):
    ...
    for batch in progress_bar(eval_dataloader):
        ...
        if preds is None:
            ...
            all_preds = np.append(preds, logits.detach().cpu().numpy(), axis=0)
    ...
    if args.do_test :
        ...
        result = compute_metrics(args.task, out_label_ids, all_preds)
    ...
    return results, all_logits, preds
...
def main(cli_args):
    ...
    args.test_file = cli_args.input_text
    args.output_dir = os.path.join(args.ckpt_dir, cli_args.output_dir)
    ...
    if args.do_eval:
        ...
        for checkpoint in checkpoints:
            ...
            if args.do_test:
                result, dev_logits, dev_preds = evaluate(args, model, test_dataset, mode="test", global_step=None)
            else :
                result, dev_logits, dev_preds = evaluate(args, model, dev_dataset, mode="dev", global_step=global_step)
