...
class MzITCProcessor(object):
    ...
    def get_labels(self):
        if self.args.task=='mzitc':
            file_to_read = self.args.intent_label
        elif self.args.task=='mzhitc':
            file_to_read = self.args.second_label
        elif self.args.task=='mzhitc_third':
            file_to_read = self.args.third_label
        lines = list(pd.read_csv(os.path.join(self.args.data_dir, file_to_read), header=None)[0].values)
        return lines
    ...
    def _create_examples(self, lines, set_type, do_test):
    ...
    def get_examples(self, mode):
        ...
        return self._create_examples(
            self._read_file(os.path.join(self.args.data_dir, file_to_read)), mode, self.args.do_test
        )

seq_cls_processors = {
    "mzitc": MzITCProcessor,
    'mzhitc': MzITCProcessor,
    'mzhitc_third': MzITCProcessor
}

seq_cls_tasks_num_labels = {"mzitc":785, 'mzhitc':139, 'mzhitc_third':684}

seq_cls_output_modes = {
    'mzitc': 'classification',
    'mzhitc': 'classification',
    'mzhitc_third': 'classification'
}
